...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        reduced = (int *)malloc(world_size * sizeof(int *));
    ...
    MPI_Reduce(result, reduced, world_size, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);

    long int reduced_sum = 0;
    for(int i = 0; i < world_size; i++){
        reduced_sum += reduced[i];
    }
